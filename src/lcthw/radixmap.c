/**
 * Based on code by Andre Reinald and then heavily modified by Zed A. Shaw
 */
...
static inline void radix_sort(short offset, uint64_t max, uint64_t *source, uint64_t *dest) {
    ...
    // count ocurrences of every byte value 
    for (sp = source, end = source + max; sp < end; sp++) {
        count[ByteOf(sp, offset)]++;
    }

    // transform count into index by summin
...
int RadixMap_delete(RadixMap *map, RMElement *el) {
    ...
    check(el != NULL, "Can't be delete a NULL element.");
