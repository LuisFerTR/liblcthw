...
static int make_random(RadixMap *map) {
    ...
    for (i = 0; i < map->max - 1; i++) {
        uint32_t key = (__uint32_t) (rand() | (rand() << 16));
...
static int test_search(RadixMap *map) {
    ...
    for (i = map->end / 2; i < map->end; i++) {
        ...
        check(found->data.key == d->data.key, "Got the worng result: %p:%u looking for %u at %u", found, found->data.key, d->data.key, i);
    }
...
static char *test_operations() {
    ...
    while (map->end > 0) {
        ...
        mu_assert(RadixMap_delete(map, el) == 0, "Didn't delete the element.");
